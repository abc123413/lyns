...
export type ResponseInputMessage =
    | AiTextMessage
    | { type: "function_call"; call_id: string; name: string; arguments: string; thoughtSignature?: string; reasoningContent?: string }
    ...
    return messages.flatMap((message): GeminiContent[] => {
        if ("type" in message) {
            if (message.type === "function_call_output") {
                callNameById.set(message.call_id, "");
                return [{ role: "user", parts: [{ functionResponse: { id: message.call_id, name: "", response: { result: jsonValue(message.output) } } }] }];
            }
...
export async function requestToolResponse(config: AiConfig, messages: ResponseInputMessage[], tools: ResponseFunctionTool[], toolChoice: ToolChoice = "auto", onDelta?: (text: string) => void, options?: RequestOptions): Promise<ToolResponseResult> {
    ...
    try {
        ...
        if (requestConfig.apiFormat === "anthropic") {
            return await requestAnthropicToolResponse(requestConfig, messages, tools, toolChoice, onDelta, options);
        }
        try {
            ...
            if (String(firstError).includes("404") || String(firstError).includes("405")) {
                return await requestChatCompletionsToolResponse(requestConfig, messages, tools, toolChoice, onDelta, options);
            }
            throw firstError;
        }
    } catch (error) {
...
async function requestChatCompletionsToolResponse(requestConfig: ReturnType<typeof resolveModelRequestConfig>, messages: ResponseInputMessage[], tools: ResponseFunctionTool[], toolChoice: ToolChoice, onDelta?: (text: string) => void, options?: RequestOptions): Promise<ToolResponseResult> {
    const ccMessages: Record<string, unknown>[] = [];
    let pendingToolCalls: Record<string, unknown>[] = [];
    let pendingReasoning = "";

    const flushPendingToolCalls = () => {
        if (!pendingToolCalls.length) return;
        const msg: Record<string, unknown> = { role: "assistant", tool_calls: pendingToolCalls };
    ...
    for (const m of messages) {
        if ("type" in m) {
            if (m.type === "function_call") {
                pendingToolCalls.push({ id: m.call_id, type: "function", function: { name: m.name, arguments: m.arguments } });
                if (m.reasoningContent) pendingReasoning = m.reasoningContent;
            } else if (m.type === "function_call_output") {
                flushPendingToolCalls();
                ccMessages.push({ role: "tool", tool_call_id: m.call_id, content: m.output });
            } else {
                flushPendingToolCalls();
                ccMessages.push({ role: "tool", tool_call_id: (m as Record<string, string>).call_id || "", content: "" });
            }
        } else if (m.role === "tool") {
            flushPendingToolCalls();
            ccMessages.push({ role: "tool", tool_call_id: (m as { tool_call_id: string }).tool_call_id || "", content: typeof m.content === "string" ? m.content : "" });
        } else {
            flushPendingToolCalls();
            ccMessages.push({ role: m.role, content: typeof m.content === "string" ? m.content : m.content?.map((p) => ("text" in p ? { type: "text" as const, text: p.text } : { type: "image_url" as const, image_url: { url: p.image_url } })) });
        }
    }
    flushPendingToolCalls();

    async function call(choice: ToolChoice) {
    ...
    let data;
    try {
        data = await call(toolChoice);
    } catch (firstError) {
        const msg = String(firstError);
        if (msg.includes("Thinking") || msg.includes("thinking") || msg.includes("tool_choice") || msg.includes("400")) {
            data = await call("auto");
        } else {
            throw firstError;
        }
    }
    ...
    const toolCalls: ResponseToolCall[] = rawToolCalls.map((tc: Record<string, unknown>) => ({
        id: String(tc.id || ""),
        type: "function" as const,
        function: { name: String((tc as Record<string, unknown>).function && (tc as Record<string, unknown>).function ? ((tc as Record<string, unknown>).function as Record<string, string>).name || "" : ""), arguments: String((tc as Record<string, unknown>).function && (tc as Record<string, unknown>).function ? ((tc as Record<string, unknown>).function as Record<string, string>).arguments || "{}" : "{}") },
        reasoningContent,
    }));
    if (onDelta && typeof content === "string") onDelta(content);
    return { content: typeof content === "string" ? content : "", toolCalls };
}

async function requestAnthropicToolResponse(requestConfig: ReturnType<typeof resolveModelRequestConfig>, messages: ResponseInputMessage[], tools: ResponseFunctionTool[], toolChoice: ToolChoice, onDelta?: (text: string) => void, options?: RequestOptions): Promise<ToolResponseResult> {
    const systemMessages: string[] = [];
    const chatMessages: Array<{ role: "user" | "assistant"; content: string | Array<{ type: string; text?: string; source?: { type: string; media_type: string; data: string } }> }> = [];

    for (const m of messages) {
        if ("type" in m) {
            if (m.type === "function_call") chatMessages.push({ role: "assistant", content: [{ type: "tool_use", id: m.call_id, name: m.name, input: JSON.parse(m.arguments || "{}") } as Record<string, unknown> as never] });
            else if (m.type === "function_call_output") chatMessages.push({ role: "user", content: [{ type: "tool_result", tool_use_id: m.call_id, content: m.output } as Record<string, unknown> as never] });
        } else if (m.role === "system") {
            systemMessages.push(typeof m.content === "string" ? m.content : "");
        } else {
            const c = typeof m.content === "string" ? m.content : "";
            chatMessages.push({ role: m.role as "user" | "assistant", content: c });
        }
    }
    ...
    if (toolChoice !== "auto") body.tool_choice = toolChoice === "required" ? { type: "any" } : { type: "tool", name: (toolChoice as { name: string }).name };

    const response = await fetch(`${requestConfig.baseUrl.replace(/\/+$/, "")}/v1/messages`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
    ...
    const content = textBlocks.map((b: Record<string, unknown>) => String(b.text || "")).join("");
    const toolCalls: ResponseToolCall[] = toolBlocks.map((b: Record<string, unknown>) => ({
        id: String(b.id || ""),
        type: "function" as const,
...
export async function fetchImageModels(config: Pick<AiConfig, "baseUrl" | "apiKey" | "apiFormat">) {
    try {
        if (config.apiFormat === "anthropic") {
            throw new Error("Anthropic 官方 API 不提供模型列表接口，请手动输入模型名称（如 claude-sonnet-4-6、claude-opus-4-7 等）");
        }
