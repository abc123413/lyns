...
import { ArrowLeft, ArrowRight, CheckSquare, Download, History, ImagePlus, LoaderCircle, Plus, SlidersHorizontal, Sparkles, Trash2, Upload } from "lucide-react";
...
const LOG_STORE_KEY = "lyns:image_generation_logs";
const RESULT_ACTION_BUTTON_CLASS = "min-w-0 px-1.5 [&_.ant-btn-icon]:shrink-0 [&>span:last-child]:min-w-0 [&>span:last-child]:truncate";
const logStore = localforage.createInstance({ name: "lyns", storeName: "image_generation_logs" });
...
export default function ImagePage() {
    ...
    const hasImageModels = effectiveConfig.imageModels.length > 0;
    const canGenerate = Boolean(prompt.trim() && hasImageModels);
                    ...
                    <div className="thin-scrollbar flex flex-col rounded-lg border border-stone-200 bg-card p-4 shadow-sm dark:border-stone-800 lg:min-h-0 lg:overflow-y-auto">
                        ...
                        <div className="mt-6 space-y-5">
                            <div>
                                ...
                                <Input.TextArea value={prompt} onChange={(event) => setPrompt(event.target.value)} rows={7} placeholder="用一句话描述你想要的画面..." />
                            </div>
                        ...
                        <div className="mt-auto pt-6">
                            {!hasImageModels ? (
                                <div className="mb-4 flex flex-col items-center gap-3 rounded-lg border border-dashed border-amber-300 bg-amber-50 px-4 py-5 text-center dark:border-amber-700 dark:bg-amber-950/30">
                                    <p className="text-sm text-amber-800 dark:text-amber-300">尚未配置生图模型，请先前往设置添加渠道和模型。</p>
                                    <Button size="small" onClick={() => openConfigDialog(false)}>前往设置</Button>
                                </div>
                            ) : null}
                            ...
                            <div className="grid gap-4 sm:grid-cols-2 2xl:grid-cols-3">
                                {results.map((result, index) =>
                                    result.status === "success" && result.image ? (
                                        <ResultImageCard key={result.id} image={result.image} index={index} onDownload={downloadImage} />
