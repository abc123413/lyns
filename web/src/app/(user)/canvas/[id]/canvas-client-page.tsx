...
import { Bot, Home, ImageIcon, Images, List, Menu, Plus, Redo2, Settings2, Trash2, Undo2, Upload } from "lucide-react";
import { saveAs } from "file-saver";

import { requestEdit, requestGeneration, requestImageQuestion } from "@/services/api/image";
import { defaultConfig, type AiConfig, useConfigStore, useEffectiveConfig } from "@/stores/use-config-store";
import { cleanupUnusedImages, resolveImageUrl, uploadImage, type UploadedImage } from "@/services/image-storage";
...
function InfiniteCanvasPage() {
    ...
    const [backgroundMode, setBackgroundMode] = useState<CanvasBackgroundMode>("white-paper");
    ...
    const cleanupCanvasFiles = useCallback(
        (extra?: unknown) => {
            cleanupUnusedImages({ nodes: nodesRef.current, history: historyRef.current, ...(extra && typeof extra === 'object' ? extra : {}) });
        },
        [],
    );
    ...
    const createAndOpenProject = useCallback(() => {
        const id = createProject(`灵思画布 ${useCanvasStore.getState().projects.length + 1}`);
    ...
    const deleteCurrentProject = useCallback(() => {
        ...
        cleanupCanvasFiles();
        router.push("/canvas");
    }, [cleanupCanvasFiles, deleteProjects, projectId, router]);
    ...
    const downloadNodeImage = useCallback((node: CanvasNodeData) => {
        ...
        saveAs(node.metadata.content, `canvas-image-${node.id}.${imageExtension(node.metadata.content)}`);
    }, []);
            ...
            try {
                if (mode === "image") {
                    ...
                    const rootNode: CanvasNodeData = {
                        id: rootId,
                        type: CanvasNodeType.Image,
                        title: effectivePrompt.slice(0, 32) || "生成的图片",
                        position: {
                ...
                if (isConfigNode || editingTextNode) {
                    const childNodes: CanvasNodeData[] = childIds.map((id, index) => ({
                        id,
                        type: CanvasNodeType.Text,
                        title: effectivePrompt.slice(0, 32) || "生成的文本",
                        position: {
                        ...
                        childIds.includes(node.id)
                            ...
                            : node.id === nodeId && isConfigNode
                              ...
                              : node.id === nodeId && !editingTextNode
                                ? { ...node, type: CanvasNodeType.Text, title: prompt.slice(0, 32) || "生成的文本", metadata: { ...node.metadata, content: answerByNodeId.get(node.id) || streamed, status: NODE_STATUS_SUCCESS } }
                                : node,
                    ),
                );
            } catch (error) {
    ...
    const insertAssistantImage = useCallback(
        async (image: CanvasAssistantImage) => {
            ...
            const node: CanvasNodeData = {
                id,
                type: CanvasNodeType.Image,
                title: image.prompt.slice(0, 32) || "生成的图片",
