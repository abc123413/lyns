...
import { Download, Ellipsis, FolderPlus, Image as ImageIcon, Info, MessageSquare, Minus, Pencil, Plus, RefreshCw, Settings2, Trash2, Upload } from "lucide-react";
...
export function CanvasNodeHoverToolbar({
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...
        ...(hasImage || isText ? [{ id: "saveAsset", title: "保存到本地", label: "保存", icon: <FolderPlus className="size-4" />, onClick: () => onSaveAsset(node) }] : []),
        ...(hasImage ? [{ id: "download", title: "下载图片", label: "下载", icon: <Download className="size-4" />, onClick: () => onDownload(node) }] : []),
                ...
                <div className="h-[56vh] min-h-[360px] text-sm">
                    {view === "info" ? (
                        <div className="thin-scrollbar h-full space-y-3 overflow-auto pr-1">
                            ...
                            <InfoRow label="状态" value={{ idle: "就绪", success: "成功", loading: "生成中", error: "失败" }[node.metadata?.status || "idle"] || node.metadata?.status} />
